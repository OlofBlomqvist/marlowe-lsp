...
use lsp_types::{*, notification::{LogMessage, DidOpenTextDocument, DidChangeTextDocument, DidCloseTextDocument, Notification}, request::{Initialize, Request, HoverRequest}};
use anyhow::*;
...
pub (crate) struct State {
    pub(crate) sources: HashMap<Url, FileId>,
    pub(crate) sexpression_asts: HashMap<Url, (Vec<(Range,sex::Rule,SemanticToken)>,ContractValidationResult)>,
    pub(crate) marlowe_asts:     HashMap<Url, (Vec<(Range,marlowe_lang::parsing::Rule,SemanticToken)>,ContractValidationResult)>,
    pub(crate) files: codespan::Files<String>,
    pub(crate) marlowe_parser_error: Option<(String,Range)>,
    pub(crate) sexpression_parser_error: Option<(String,Range)>,
    // because we might want to start emitting new types of items from inside whatever method whenever,
    // without having to do breaking api changes, we just make people consume from here.
    pub(crate) outgoing_log_messages: VecDeque<(MessageType,String)>,
    pub(crate) outgoing_diagnostics: VecDeque<PublishDiagnosticsParams>
}

pub struct MarloweLSPServer{
...
impl  MarloweLSPServer {
    ...
    pub fn hover(&mut self, params: HoverParams) -> Result<Option<Hover>> {
        ...
        match self.state.marlowe_asts.get(&params.text_document_position_params.text_document.uri) {
            Some(token_list) => {
    ...
    pub fn formatting(&mut self, params: DocumentFormattingParams) -> Result<Option<Vec<TextEdit>>> {
        self.state.outgoing_log_messages.push_back((MessageType::INFO,"formatting a document".into()));
        let id = *self.state.sources.get(&params.text_document.uri).unwrap();
        let mut source = self.state.files.source(id).to_owned();
        let new_text = marlowe_lang::parsing::fmt::fmt(&source);
        Ok(Some(vec![
        TextEdit {
    ...
    pub  fn create_log_message(message:LogMessageParams) -> serde_json::Value {
        json!({
            "jsonrpc": "2.0",
    ...
    pub fn document_highlight(
        ...
        let toks = {
            match self.state.marlowe_asts.get_mut(&params.text_document_position_params.text_document.uri) {
                None => vec![],
        ...
        match closest {
            Some((a,rule,_c)) => {
                self.state.outgoing_log_messages.push_back((MessageType::INFO, format!("highlighting selected '{rule:?}'").into()));
                Ok(Some(vec![
                    DocumentHighlight { 
                        range: a,
        ...
        // self.state.outgoing_log_messages.push_back((MessageType::INFO, "highlighed stuff".into()));

        // Ok(
        //     vec![
        //         DocumentHighlight { 
        //             range: Range { start: Position { line: 0, character: 0 }, end: Position { line: 1, character: 99 } },
        //             kind: Some(DocumentHighlightKind::TEXT)
        //         }
        //     ]
        // )
    ...
    pub fn shutdown(&mut self) -> Result<()> {  
        self.state.outgoing_log_messages.push_back((MessageType::INFO, "shutdown!".into()));
        Ok(()) 
    }

    pub fn execute_command(&self, p: ExecuteCommandParams) -> Result<serde_json::Value> {
        Ok(Self::create_log_message(LogMessageParams { typ: MessageType::INFO, message: "execute command ok".into() }))
    }

    pub fn did_open(&mut self, params: DidOpenTextDocumentParams) {
        get_or_insert_document(&mut self.state, &params.text_document);
        let result =  get_diagnostics(&mut self.state,&params.text_document.uri);

        let thing = PublishDiagnosticsParams::new(
            params.text_document.uri.clone(), 
            result, 
            Some(params.text_document.version)
        );
        self.state.outgoing_diagnostics.push_back(thing);
        self.state.outgoing_log_messages.push_back((MessageType::INFO,"a new document was opened".into()));
    ...
    pub fn did_change(&mut self, params: DidChangeTextDocumentParams) {
        ...
        update_document(&mut self.state, &params.text_document.uri, params.content_changes);
        let result = get_diagnostics(&mut self.state,&params.text_document.uri);
        let thing = PublishDiagnosticsParams::new(
            params.text_document.uri.clone(), 
            result, 
        ...
        self.state.outgoing_log_messages.push_back((MessageType::INFO,"detected change".into()));
    ...
    pub fn did_close(&mut self, params: DidCloseTextDocumentParams) {
        self.state.marlowe_asts.remove(&params.text_document.uri);
        self.state.sexpression_asts.remove(&params.text_document.uri);
    ...
    pub fn completion(&mut self, completion_params: CompletionParams) -> Result<Option<CompletionResponse>> {
        ...
        let (source,col) = {
            let id = *self.state.sources.get(&completion_params.text_document_position.text_document.uri).unwrap();
            let bindex = codespan_lsp_local::position_to_byte_index(
                &self.state.files, id, &completion_params.text_document_position.position).unwrap();
            let src = self.state.files.source(id).to_owned();
            (src,bindex)
        };
    ...
    fn handle_request<T, U, F>(map: &mut serde_json::Map<std::string::String, serde_json::Value>, f: F) -> Result<Option<Value>>
    where
        ...
        F: FnOnce(T) -> Result<U>,
    {
        if let Some(params) = map.remove("params") {
            match serde_json::from_value::<T>(params) {
                std::result::Result::Ok(tparams) => {
                    let result = f(tparams)?;
                    match map.remove("id") {
                        Some(id) => {
                            let response = json!({
                                "jsonrpc": "2.0",
            ...
            Err(anyhow::Error::msg("Message missing params"))
        }
    
    }

    fn handle_request_silent<T, U, F>(map: &mut serde_json::Map<std::string::String, serde_json::Value>, f: F) -> Result<Option<Value>>
    where
        T: serde::de::DeserializeOwned,
        U: serde::Serialize,
        F: FnOnce(T) -> (),
    ...
    // we can probably use this with both cli and wasm ?
    pub fn run_with_json(&mut self,body_str:&str) -> Result<Option<serde_json::Value>> {
        match serde_json::from_str::<Value>(&body_str) {
            std::result::Result::Ok(Value::Object(mut map)) => {
                if let Some(Value::String(method)) = map.remove("method") {
                    match method.as_str() {
                        Initialize::METHOD => {
                            Self::handle_request::<InitializeParams, InitializeResult, _>(&mut map, |params| {
                                self.initialize(params)
                            })
                        },    
                        DidOpenTextDocument::METHOD => {
                            Self::handle_request_silent::<DidOpenTextDocumentParams,Option<()>,_>(&mut map, |params| {
                        ...
                        DidCloseTextDocument::METHOD => {
                            Self::handle_request_silent::<DidCloseTextDocumentParams,Option<()>,_>(&mut map, |params| {
                                self.did_close(params)
                            })
                        },                        
                        HoverRequest::METHOD => {
                            Self::handle_request::<HoverParams,Option<Hover>, _>(&mut map, |params| {
                                self.hover(params)
                            })
                        },                        
                        "textDocument/documentHighlight" => {
                            // Handle DocumentHighlight request
                            // The params for this request are of type DocumentHighlightParams
                            Self::handle_request::<DocumentHighlightParams,Option<Vec<DocumentHighlight>>, _>(&mut map, |params| {
                                self.document_highlight(params)
                            })
                        },
                        "textDocument/formatting" => {
                            Self::handle_request::<DocumentFormattingParams,Option<Vec<TextEdit>>, _>(&mut map, |params| {
                                self.formatting(params)
                            })
                        },
                        "textDocument/completion" => {
                            Self::handle_request::<CompletionParams,Option<CompletionResponse>, _>(&mut map, |params| {
                                self.completion(params)
                            })
                        }
                        "textDocument/semanticTokens/full" => {
                            // Handle SemanticTokens request
                            // The params for this request are of type SemanticTokensParams
                            Self::handle_request::<SemanticTokensParams,Option<SemanticTokensResult>, _>(&mut map, |params| {
                                self.semantic_tokens_full(params)
                            })
                        },
                        _ => {
                            self.state.outgoing_log_messages.push_back((lsp_types::MessageType::WARNING,format!("Unknown method received: {}",method)));
                            Ok(None)
                        }
                    }
                } else {
                    Err(anyhow::Error::msg("unknown message!"))                
                }
            },
            std::result::Result::Ok(x) => Err(anyhow::Error::msg("unknown message, missing method info!"))      ,
            Err(e) => Err(e.into())      
        }
    }
...
fn update_asts(source:String,state:&mut State,url:Url)  {
    ...
    let mar_vec = 
        match &marlowe_tokens {
            std::result::Result::Ok(x) => x.0.to_vec(),
            Err(_) => vec![],
        };
...
#[derive(Debug)]
pub (crate) struct ContractValidationResult {
...
// todo - move most of this to marlowe-rs?
#[decurse::decurse]
                ...
                match timeout.as_rule() {
                    Rule::TimeConstant => {
                        match timeout.as_str().parse::<i64>() {
                            std::result::Result::Ok(this_timeout_value) => {
            ...
            fn lsp_parse(sample:String,f: impl Fn($rule_type,Range) -> u32) -> 
                ...
                match <$parser_type>::parse(
                    ...
                    std::result::Result::Ok(p) => { 
                        ...
                        std::result::Result::Ok((data,validation_result))
